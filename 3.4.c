...
int main(void){
	while(1){
		...
		if(pid==0){
			...
			sleep(PIN_WAIT_INTERVAL); //delaying PIN eneration intentionally.
			exit(EXIT_SUCCESS);
		}

		if(pid>0){
			wait(NULL); //waiting for child to  fisish
			close(pipefds[1]);// close write fd
			read(pipefds[1], buffer, PIN_LENGTH + 1); //read PIN from pipe
			close(pipefds[0]);//close read fd
